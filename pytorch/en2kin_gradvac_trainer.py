...
def valid_loop(model: En2KinTransformer, engl_roberta_model : Union[RobertaModel,None], kin_lm_model: Union[MyGPTEncoder,None],
...
def train_loop(model: En2KinTransformer, engl_roberta_model : Union[RobertaModel,None], kin_lm_model: Union[MyGPTEncoder,None],
               device, grad_optimizer: GradVac, lr_scheduler: InverseSQRT_LRScheduler, data_loader,
...
def train_fn(rank, args, cfg:BaseConfig, data_train, data_valid):
    ...
    optmz = apex.optimizers.FusedAdam(model.parameters(), lr=peak_lr, betas=(0.9, 0.98), eps=1e-08)
    grad_optimizer = GradVac((5 if args.kinmt_use_copy_loss else 4), optmz, device, beta=1e-2, reduction='sum', cpu_offload = False)
    # grad_optimizer = GradVac((5 if args.kinmt_use_copy_loss else 4), optmz, device, scaler=scaler, beta=1e-2, reduction='sum', cpu_offload = False)
    ...
    lr_scheduler = InverseSQRT_LRScheduler(optmz,
                                           start_lr=peak_lr,
    ...
    lr_scheduler.warmup_iter = args.kinmt_warmup_steps
    ...
    with progressbar.ProgressBar(initial_value=init_epoch,
        ...
        for epoch in range(init_epoch,args.kinmt_num_train_epochs):
            total_steps = train_loop(model, engl_roberta_model, kin_lm_model,
                                     device, grad_optimizer, lr_scheduler, train_data_loader,
            ...
            if dist.get_rank() == 0:
                ...
                bar.update(epoch + 1)
                sys.stdout.flush()
            train_dataset.shuffle_buckets_and_mark_batches()
