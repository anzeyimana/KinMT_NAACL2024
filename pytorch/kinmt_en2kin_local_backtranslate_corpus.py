...
from kinmt_decode_en2kin import en2kin_init_models, read_english_lexicon, en2kin_translate, reset_and_print_en2kin_latency_info
...
def translate_english_corpus(args, input_filename:str, output_filename:str, expected_length:int):
    ...
    en2kin_model_setup = en2kin_init_models(args, rank=rank, socket_synthesis=True, morpho_conf = f'config_morpho.conf')
    ...
    to_skip = 0
    with open(output_filename, 'r', encoding='utf-8') as test_file:
        for kin_sent in test_file:
            to_skip += 1
            ...
            with progressbar.ProgressBar(max_value=(expected_length), redirect_stdout=True) as bar:
                for engl_sentence in in_file:
                    if (counter % 100 == 0):
                        ...
                        reset_and_print_en2kin_latency_info()
                        bar.update(counter)
                    if counter >= to_skip:
                        ...
                        if len(engl_sentence) > 0:
                            ...
                            if len(complete_translations) > 0:
                                ...
                                print(f'!Failed to translate \'{engl_sentence}\'')
...
if __name__ == '__main__':
    ...
    input_filename = f'kinmt/parallel_data_2022/txt/english_corpus_2023-06-10_T{args.corpus_id}_en.txt'
    output_filename = f'kinmt/parallel_data_2022/txt/english_corpus_2023-06-10_T{args.corpus_id}_rw.txt'
    expected_length = 1_600_000
