...
from morphoy import build_morphoy_lib, parse_text_to_morpho_sentence, ParsedMorphoSentence
...
def kin2en_init_models(args, rank=0, use_morpho_lib = True):
    ffi, lib = None, None
    if use_morpho_lib:
        build_morphoy_lib()
    ...
    english_decoder = TransformerLanguageModel.from_pretrained('wmt19.en', 'model.pt', tokenizer='moses', bpe='fastbpe').cpu()
    ...
    return (ffi, lib, cfg, device, kin2en_model, mybert_encoder, english_lm, english_decoder)
...
def parse_kinya_sentence(input_sentence: str, ffi, lib, device, cfg: BaseConfig, attempt_auto_correction=False):
    predict_affixes = False
    if lib is not None:
        parsed_sentence = parse_text_to_morpho_sentence(ffi, lib, input_sentence, attempt_auto_correction=attempt_auto_correction)
    else:
...
def kin2en_translate(model_setup,
                     kinya_sentence,
                     beam_size, max_eval_sequences, max_text_length,
                     ready_sentence_item=False,
                     copy_threshold=0.5, intersect_threshold=0.5,
    ...
    (ffi, lib, cfg, device, kin2en_model, mybert_encoder, english_lm, english_decoder) = model_setup
    kin2en_model.eval()
    english_decoder.eval()
    ...
    if ready_sentence_item:
        seed_data_item, kinya_bpe_dict = kinya_sentence, None
    else:
        seed_data_item, kinya_bpe_dict = parse_kinya_sentence(kinya_sentence, ffi, lib, device, cfg, attempt_auto_correction=attempt_auto_correction)

    TRANSLATION_SENTENCE_COUNT += 1.0
    TRANSLATION_TOKEN_COUNT += (1.0*seed_data_item[2].shape[0])
