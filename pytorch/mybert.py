...
from modules import PositionEncoding, ClassificationHead, TokenClassificationHead, MorphoEncoder, MorphoMLMPredictor, BaseConfig
...
class MyBERT_PretrainModel(nn.Module):
    def __init__(self, args, cfg:BaseConfig):
        super(MyBERT_PretrainModel, self).__init__()
        self.encoder = MyBERTEncoder(args, cfg)
...
class MyBERT_SequenceClassifier(nn.Module):

    def __init__(self, args, cfg:BaseConfig, num_classes: int, encoder:MyBERTEncoder= None):
        super(MyBERT_SequenceClassifier, self).__init__()
...
class MyBERT_SequenceTagger(nn.Module):

    def __init__(self, args, cfg:BaseConfig, num_classes: int, encoder:MyBERTEncoder= None):
        super(MyBERT_SequenceTagger, self).__init__()
        self.encoder_fine_tune = args.encoder_fine_tune
        self.hidden_dim = (args.morpho_dim_hidden * 4) + args.stem_dim_hidden  # 128 x 4 + 256 = 768
        if (encoder is not None):
           self.encoder = encoder
...
def MyBERT_from_pretrained(device, args, cfg: BaseConfig, pretrained_model_file) -> MyBERT_PretrainModel:
    pretrained_model = MyBERT_PretrainModel(args, cfg).to(device)
...
def MyBERT_large_from_pretrained(device, args, cfg: BaseConfig, pretrained_model_file) -> MyBERT_PretrainModel:
...
def MyBERT_SequenceClassifier_from_pretrained(num_classes, device, args, cfg: BaseConfig, pretrained_model_file, pre_trained_model=None) -> MyBERT_SequenceClassifier:
    classifier_model = MyBERT_SequenceClassifier(args, cfg, num_classes).to(device)
...
def MyBERT_SequenceTagger_from_pretrained(num_classes, device, args, cfg: BaseConfig, pretrained_model_file) -> MyBERT_SequenceTagger:
    tagger_model = MyBERT_SequenceTagger(args, cfg, num_classes).to(device)
    if args.encoder_fine_tune:
        kb_state_dict = torch.load(pretrained_model_file, map_location=device)
        pretrained_model = MyBERT_PretrainModel(args, cfg).to(device)
        pretrained_model.load_state_dict(kb_state_dict['model_state_dict'])
