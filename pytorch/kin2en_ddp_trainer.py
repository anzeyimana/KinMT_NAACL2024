...
from kinmt_data import KINMTDataCollection, KINMTDataset, kinmt_kin2en_data_collate_fn, \
    read_parallel_data_files, pick_first_collate_fn
    ...
    with torch.no_grad():
        for batch_idx, batch_data_item in enumerate(data_loader):
            with torch.cuda.amp.autocast():
                losses, nll_losses = model(mybert_encoder, english_lm, batch_data_item) #copy_batch_item_to_device(batch_data_item, device))
...
def kin2en_trainer_main():
    ...
    data_train = read_parallel_data_files("kinmt/parallel_data_2022/txt", train_data, 512, english_ext='_lm_en.txt')
    data_valid = read_parallel_data_files("kinmt/parallel_data_2022/txt", valid_data, 512, english_ext='_lm_en.txt')
