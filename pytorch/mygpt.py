...
from morphoy import parse_text_to_morpho_sentence, ParsedMorphoSentence, BOS_ID, EOS_ID
from kb_transformers import TransformerDecoderLayer, TransformerDecoder, init_bert_params
from modules import PositionEncoding, MorphoEncoder, MorphoGPTPredictor, BaseConfig, \
    ClassificationHead, GPTClassificationHead
...
class MyGPTEncoder(nn.Module):
    def __init__(self, args, cfg:BaseConfig):
        super(MyGPTEncoder, self).__init__()
...
class MyGPT(nn.Module):
    def __init__(self, args, cfg:BaseConfig):
        ...
        self.encoder = MyGPTEncoder(args, cfg)
...
class MyGPT_SequenceClassifier(nn.Module):

    def __init__(self, args, cfg:BaseConfig, num_classes: int, encoder:MyGPTEncoder= None):
        super(MyGPT_SequenceClassifier, self).__init__()
...
def MyGPT_from_pretrained(args, cfg:BaseConfig, pretrained_model_file):
    gpt_model = MyGPT(args,cfg)
...
def MyGPT_SequenceClassifier_from_pretrained(num_classes, device, args, cfg: BaseConfig, pretrained_model_file) -> MyGPT_SequenceClassifier:
    classifier_model = MyGPT_SequenceClassifier(args, cfg, num_classes).to(device)
    if args.encoder_fine_tune:
        pretrained_model = MyGPT_from_pretrained(args, cfg, pretrained_model_file)
