 ...
 */
public class GazettePDFTextStripper extends LegacyPDFStreamEngine {
    ...
    private static final Log LOG = LogFactory.getLog(GazettePDFTextStripper.class);
    ...
    static {
        ...
        try {
            String className = GazettePDFTextStripper.class.getSimpleName().toLowerCase();
    ...
    /**
     * Instantiate a new GazettePDFTextStripper object.
     *
     * @throws IOException If there is an error loading the properties.
     */
    public GazettePDFTextStripper() throws IOException {
    }

    public GazettePDFTextStripper(FileWriter fileOut) throws IOException {
        this.fileOut = fileOut;
        this.printToFile = true;
        this.printToSystemOut = false;
    }
    ...
    public void printText(PDDocument doc, int minPage, int maxPage) throws IOException {
        resetEngine();
        document = doc;
        ...
        printPages(document.getPages(), minPage, maxPage);
        endDocument(document);
        if (printToFile && (fileOut != null)) {
            fileOut.flush();
            try {
                fileOut.close();
            } catch (Exception ex) {
    ...
    protected void printPages(PDPageTree pages, int min, int max) throws IOException {
        ...
        for (PDPage page : pages) {
            currentPageNo++;
            if ((currentPageNo >= min) && (currentPageNo <= max)) {
                if (page.hasContents()) {
                    if (printToSystemOut) {
    ...
    public void printPage(PDPage page) throws IOException {
        if (page.getCropBox() != null) {
            ...
            pageWidth = page.getBleedBox().getWidth();
...
//        output.print(getArticleEnd());
    ...
    protected void printPage() throws IOException {
        ...
        for (List<TextPosition> textList : charactersByArticle) {
            ...
            while (textIter.hasNext()) {
                ...
                if (position.getTextMatrix() != null) {
                    dx = position.getTextMatrix().getTranslateX() - tx;
                    dy = position.getTextMatrix().getTranslateY() - ty;
                    tx = position.getTextMatrix().getTranslateX();
                    ty = position.getTextMatrix().getTranslateY();
                }
                ...
                if (lastPosition != null) {
                    ...
                    if (!overlap(positionY, positionHeight, maxYForLine, maxHeightForLine)) {
                        List<WordWithTextPositions> myLine = normalize(line);
                        int numberOfStrings = myLine.size();
                        for (int i = 0; i < numberOfStrings; i++) {
                            String text = myLine.get(i).getText().
                                    trim().
                                    replace("  ", " ").
                                    replace("  ", " ").
                                    replace("  ", " ").
                                    replace("  ", " ");
                            boolean ok = true;
                            if (text.startsWith("Official Gazette n")) {
...
//                        if ((Math.abs(dx) >= 300.0) || (Math.abs(dy) >= 200.0))
//                        if ((Math.abs(dx) >= (pageWidth / 2.9)) || (Math.abs(dy) >= (pageHeight / 5.0)))
//                        if ((Math.abs(dx) >= (pageWidth / 2.9)) || (Math.abs(dy) >= (pageHeight / 5.0)) || (dy >= 50.0)) {
                        if ((dx >= (pageWidth / 2.0f)) || (Math.abs(dy) >= (pageHeight / 5.0f)) || (dy >= 20.0f)) {
//                            System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~> dx: " + nf.format(dx) + ", dy: " + nf.format(dy) + ", PH: " + nf.format(pageHeight));
                            printSection(outBld.toString());
                            outBld = new StringBuilder();
                        }
                        line.clear();
                ...
                if (characterValue != null) {
                    if (startOfPage && lastPosition == null) {
                        printParagraphStart();// not sure this is correct for RTL?
                    }
    ...
    public void printSection(String str) {
        PlainSimpleTokenizer tok = new PlainSimpleTokenizer(str, false);
        String t;
        ...
        while ((t = tok.nextToken()) != null) {
            if ((!(".".equals(prev) && (".".equals(t)))) &&
                    (!("…".equals(prev) && ("…".equals(t))))) {
                if (OrthographyTool.measureWordCorrectness(t.toLowerCase()) >= 10) {
                    kin++;
                }
                tot++;
                strBld.append(" ").append(t);
            }
            prev = t;
        }
        if ((tot >= 8) && ((((double) kin) / ((double) tot)) >= 0.4)) {
            if (printToSystemOut) {
                String myText = strBld.toString();
//                if (str.contains("Official Gazette")) {
//                    System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n" + myText);
//                }
                System.out.println(myText);
            } else if (printToFile && (fileOut != null)) {
                try {
                    fileOut.write(strBld.toString() + "\n");
                    fileOut.flush();
                } catch (Exception ex) {
                    System.out.println("File printing error: " + ex);
                }
            }
        }
//        System.out.println("++++++++++++++++++++++++++++++++++++> tot: " + tot + ", kin: " + kin + ", ratio: " + (((double) kin) / ((double) tot)));
//        System.out.println(strBld.toString());
     ...
     */
    protected void printLineSeparator() throws IOException {
//        output.print(getLineSeparator());
    }
     ...
     */
    protected void printWordSeparator() throws IOException {
//        output.print(getWordSeparator());
    }

    /**
     * Write the string in TextPosition to the output stream.
     *
     * @param text The text to print to the stream.
     * @throws IOException If there is an error when writing the text.
     */
    protected void printCharacters(TextPosition text) throws IOException {
//        output.print(text.getUnicode());
    }

    /**
     * Write a Java string to the output stream. The default implementation will ignore the <code>textPositions</code>
     * and just calls {@link #printString(String)}.
     *
     * @param text          The text to print to the stream.
     * @param textPositions The TextPositions belonging to the text.
     * @throws IOException If there is an error when writing the text.
     */
    protected void printString(String text, List<TextPosition> textPositions) throws IOException {
        printString(text);
    }
     ...
     * @throws IOException If there is an error when writing the text.
     */
    protected void printString(String text) throws IOException {
//        output.write(text);
    }
     ...
     * example, a PDF writer may print out all text by font, so all bold or larger text, then make a second pass and
     ...
     * @throws IOException if something went wrong
     */
    protected void printParagraphSeparator() throws IOException {
        printParagraphEnd();
        printParagraphStart();
    }
     ...
     * @throws IOException if something went wrong
     */
    protected void printParagraphStart() throws IOException {
        if (inParagraph) {
            printParagraphEnd();
            inParagraph = false;
        }
//        output.print(getParagraphStart());
        inParagraph = true;
    }
     ...
     * @throws IOException if something went wrong
     */
    protected void printParagraphEnd() throws IOException {
        if (!inParagraph) {
            printParagraphStart();
        }
//        output.print(getParagraphEnd());
        inParagraph = false;
    }
...
//        output.print(getPageStart());
     ...
     * @throws IOException if something went wrong
     */
    protected void printPageEnd() throws IOException {
//        output.print(getPageEnd());
    }
     ...
     * @throws IOException if something went wrong
     */
    private void printLine(List<WordWithTextPositions> line)
            throws IOException {
        int numberOfStrings = line.size();
        for (int i = 0; i < numberOfStrings; i++) {
//            System.out.println(">>>>>>>>>>>>>>>>>> " + line.get(i).getText());
            WordWithTextPositions word = line.get(i);
            printString(word.getText(), word.getTextPositions());
    ...
    static {
        ...
        InputStream input = GazettePDFTextStripper.class.getClassLoader().getResourceAsStream(path);
        try {
            if (input != null) {
                parseBidiFile(input);
            } else {
